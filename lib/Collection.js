...
decaf.extend(Collection.prototype, {
    ...
    ensureIndex: function (keys, options) {
        ...
        if (options) {
            ...
            this.handle.ensureIndex(keys, options);
        }
        else {
            return this.handle.ensureIndex(keys);
        }
//        return options ? this.handle.ensureIndex(serialize(keys), serialize(options)) : this.handle.ensureIndex(serialize(keys));
    ...
    insert: function (o) {
        ...
        return Json.parse(o); // BSON.from(o); // JSON.parse(o);
    },
    ...
    findOne: function (o, projection) {
        ...
        return Json.parse(result.toString()); // JSON.parse(result.toString());
    },
